...
#define _BETWEEN(a, c, b) ( (c) - (a) >= 0 && (c) <= (b) )
...
int utf8_decodeBytesCount(uint8_t byte) {
  ...
  if ((byte >> 7) == 0x0) return 1;
  if ((byte >> 6) == 0x2) return 1; //< continuation byte
  if ((byte >> 5) == 0x6) return 2;
  if ((byte >> 4) == 0xE) return 3;
...
int utf8_encodeValue(int value, uint8_t* bytes) {
  ...
  if (value <= 0x7ff) {
    *(bytes++) = (uint8_t)(0xC0 | ((value & 0x7C0) >> 6));
    *(bytes) = (uint8_t)(0x80 | ((value & 0x3F)));
    return 2;
  }
  ...
  if (value <= 0xffff) {
    *(bytes++) = (uint8_t)(0xE0 | ((value & 0xF000) >> 12));
    *(bytes++) = (uint8_t)(0x80 | ((value & 0xFC0 ) >> 6));
    *(bytes) =   (uint8_t)(0x80 | ((value & 0x3F)));
    return 3;
  }
  ...
  // first byte.
  if (value <= 0x10ffff) {
    *(bytes++) = (uint8_t)(0xF0 | ((value & (0x7    << 18)) >> 18));
    *(bytes++) = (uint8_t)(0x80 | ((value & (0x3F << 12)) >> 12));
    *(bytes++) = (uint8_t)(0x80 | ((value & (0x3F << 6))  >> 6));
    *(bytes)   = (uint8_t)(0x80 | ((value &  0x3F)));
    return 4;
  }
