 ...
 * Patch Version - Increment when bug fixed or minor changes between
 *   releases. */
 ...
 * shared library and define PK_COMPILE to export symbols when compiling
 * the pocketlang it self as a shared library. */
...
/* Returns the docstring of the function, which is a static const char*
 ...
 * _pk_doc_<fn> = docstring; That'll used to generate function help
 * text.  [signature] is the function name and parameter names with type
 * information.  ex: `io.open(path:String, mode:String) -> io.File` */
...
/* PocketLang Virtual Machine. It'll contain the state of the execution,
 * stack, heap, and manage memory allocations.
 */
typedef struct PKVM PKVM;

/* A handle to the pocketlang variables. It'll hold the reference to the
 * variable and ensure that the variable it holds won't be garbage
 * collected till it's released with pkReleaseHandle().
 */
 ...
 *
 * - To allocate new memory it'll pass NULL to parameter [memory] and
 *   the required size to [new_size]. On failure the return value would
 *   be NULL.
 *
 * - When reallocating an existing memory if it's grow in place the
 *   return address would be the same as [memory] otherwise a new
 *   address.
 *
...
/* A function callback to read a line from stdin. The returned string
 * shouldn't contain a line ending (\n or \r\n). The returned string
 * **must** be allocated with pkRealloc() and the VM will claim the
 * ownership of the string.
 */
typedef char* (*pkReadFn) (PKVM* vm);

/* A generic function thiat could be used by the PKVM to signal
 * something to the host application. The first argument is depend on
 * the callback it's registered.
 */
typedef void (*pkSignalFn) (void*);

/* Load and return the script. Called by the compiler to fetch initial
 * source code and source for import statements. Return NULL to indicate
 * failure to load. Otherwise the string **must** be allocated with
...
/* Load and return the native extension (*.dll, *.so) from the path,
 * this will then used to import the module with the pkImportImportDL
 * function. On error the function should return NULL and shouldn't use
 * any error api function.
 */
typedef void* (*pkLoadDL) (PKVM* vm, const char* path);

/* Native extension loader from the dynamic library. The handle should
 * be vaiid as long as the module handle is alive. On error the function
 * should return NULL and shouldn't use any error api function.
 */
typedef PkHandle* (*pkImportDL) (PKVM* vm, void* handle);

/* Once the native module is gargage collected, the dl handle will be
 * released with pkUnloadDL function.
 */
typedef void (*pkUnloadDL) (PKVM* vm, void* handle);

#endif // PK_NO_DL

/* A function callback to resolve the import statement path. [from] path
 * can be either path to a script or a directory or NULL if [path] is
 * relative to cwd. If the path is a directory it'll always ends with a
 * path separator which could be either '/' or '\\' regardless of the
 * system. Since pocketlang is un aware of the system, to indicate that
 * the path is a directory.
 *
 * The return value should be a normalized absolute path of the [path].
 * Return NULL to indicate failure to resolve. Othrewise the string
 * **must** be allocated with pkRealloc() and the VM will claim the
 * ownership of the string.
 */
...
/* A function callback to allocate and return a new instance of the
 * registered class. Which will be called when the instance is
 * constructed. The returned/ data is expected to be alive till the
 * delete callback occurs.
 */
typedef void* (*pkNewInstanceFn) (PKVM* vm);

/* A function callback to de-allocate the allocated native instance of
 * the registered class. This function is invoked at the GC execution.
 * No object allocations are allowed during it, so **NEVER** allocate
...
struct PkConfiguration {
  ...
  /* The callback used to allocate, reallocate, and free. If the
   * function pointer is NULL it defaults to the VM's realloc(), free()
   * wrappers. */
...
/* Register a new builtin function with the given [name]. [docstring]
 * could be NULL or will always valid pointer since PKVM doesn't
 * allocate a string for docstrings. */
PK_PUBLIC void pkRegisterBuiltinFn(PKVM* vm, const char* name,
	pkNativeFn fn, int arity, const char* docstring);

/* Adds a new search paht to the VM, the path will be appended to the
 * list of search paths. Search path orders are the same as the
 * registered order.  the last character of the path **must** be a path
 * seperator '/' or '\\'.  */
PK_PUBLIC void pkAddSearchPath(PKVM* vm, const char* path);

/* Invoke pocketlang's allocator directly.  This function should be
 * called when the host application want to send strings to the PKVM
 * that are claimed by the VM once the caller returned it. For other
 * uses you **should** call pkRealloc with [size] 0 to cleanup,
...
/* Release the handle and allow its value to be garbage collected.
 * Always call this for every handles before freeing the VM.  */
PK_PUBLIC void pkReleaseHandle(PKVM* vm, PkHandle* handle);

/* Add a new module named [name] to the [vm]. Note that the module
 * shouldn't already existed, otherwise an assertion will fail to
 * indicate that.  */
...
/* Add a native function to the given module. If [arity] is -1 that
 * means the function has variadic parameters and use pkGetArgc() to get
 * the argc.  Note that the function will be added as a global variable
 * of the module.  [docstring] is optional and could be omitted with
 * NULL.  */
...
/* Create a new class on the [module] with the [name] and return it.  If
 * the [base_class] is NULL by default it'll set to "Object" class.
...
/* Add a native method to the given class. If the [arity] is -1 that
 * means the method has variadic parameters and use pkGetArgc() to get
 * the argc.  [docstring] is optional and could be omitted with NULL. */
...
/* FIXME: Currently exit function will terminate the process which
 * should exit from the function and return to the caller.
 *
 * Run pocketlang REPL mode. If there isn't any stdin read function
 * defined, or imput function ruturned NULL, it'll immediatly return a
 * runtime error.  */
...
/* Return the current functions argument count. This is needed for
 * functions registered with -1 argument count (which means variadic
 * arguments). */
PK_PUBLIC int pkGetArgc(const PKVM* vm);

/* Check if the argc is in the range of (min <= argc <= max), if it's
 * not, a runtime error will be set and return false, otherwise return
 * true. Assuming that min <= max, and pocketlang won't validate this in
 * release binary. */
PK_PUBLIC bool pkCheckArgcRange(PKVM* vm, int argc, int min, int max);

/* Helper function to check if the argument at the [slot] slot is
 * Boolean and if not set a runtime error. */
PK_PUBLIC bool pkValidateSlotBool(PKVM* vm, int slot, bool* value);

/* Helper function to check if the argument at the [slot] slot is Number
 * and if not set a runtime error. */
PK_PUBLIC bool pkValidateSlotNumber(PKVM* vm, int slot, double* value);

/* Helper function to check if the argument at the [slot] is an a whold
 * number and if not set a runtime error. */
...
/* Helper function to check if the argument at the [slot] slot is of
 * type [type] and if not sets a runtime error. */
PK_PUBLIC bool pkValidateSlotType(PKVM* vm, int slot, PkVarType type);

/* Helper function to check if the argument at the [slot] slot is an
 * instance of the class which is at the [cls] index. If not set a
 * runtime error. */
PK_PUBLIC bool pkValidateSlotInstanceOf(PKVM* vm, int slot, int cls);

/* Helper function to check if the instance at the [inst] slot is an
 * instance of the class which is at the [cls] index. The value will be
 * set to [val] if the object at [cls] slot isn't a valid class a
 * runtime error will be set and return false. */
PK_PUBLIC bool pkIsSlotInstanceOf(PKVM* vm, int inst, int cls, bool* val);

/* Make sure the fiber has [count] number of slots to work with
 * (including the arguments). */
PK_PUBLIC void pkReserveSlots(PKVM* vm, int count);

/* Returns the available number of slots to work with. It has at least
 * the number argument the function is registered plus one for return
 * value. */
...
/* Returns boolean value at the [index] slot. If the value at the
 * [index] is not a boolean it'll be casted (only for booleans). */
...
/* Returns the string at the [index] slot. The returned pointer is only
 * valid inside the native function that called this. Afterwards it may
 * garbage collected and become demangled. If the [length] is not NULL
 * the length of the string will be written. */
PK_PUBLIC const char* pkGetSlotString(PKVM* vm, int index,
	uint32_t* length);

/* Capture the variable at the [index] slot and return its handle. As
 * long as the handle is not released with `pkReleaseHandle()` the
 * variable won't be garbage collected. */
...
/* Returns the native instance at the [index] slot. If the value at the
 * [index] is not a valid native instance, an assertion will fail. */
...
/* Create a new String copying the [value] and set it to [index]
 * slot. */
...
/* Create a new string copying from the formated string and set it to
 * [index] slot. */
PK_PUBLIC void pkSetSlotStringFmt(PKVM* vm, int index,
	const char* fmt, ...);
...
/* Returns the hash of the [index] slot value. The value at the [index]
 * must be hashable. */
...
/* Creates a new instance of class at the [cls] slot, calls the
 * constructor, and place it at the [index] slot. Returns true if the
 * instance constructed successfully.
 *
 * [argc] is the argument count for the constructor, and [argv] is the
 * first argument slot's index. */
PK_PUBLIC bool pkNewInstance(PKVM* vm, int cls, int index, int argc, int argv);

PK_PUBLIC bool pkCreateInstance( PKVM* vm, int cls, int index, void* data );
...
/* Insert [value] to the [list] at the [index], if the index is less
 * than zero, it'll count from backwards. ie. insert[-1] ==
 * insert[list.length].  Note that slot [list] must be a valid list
 * otherwise it'll fail an assertion. */
PK_PUBLIC bool pkListInsert(PKVM* vm, int list, int32_t index, int value);

/* Pop an element from [list] at [index] and place it at the [popped]
 * slot, if [popped] is negative, the popped value will be ignored. */
PK_PUBLIC bool pkListPop(PKVM* vm, int list, int32_t index, int popped);

/* Get an element from [list] at [index] and place it at the [value]
 * slot. */
PK_PUBLIC bool pkListGet(PKVM* vm, int list, int32_t index, int value);

/* Returns the length of the list at the [list] slot, it the slot isn't
 * a list an assertion will fail. */
...
/* Calls a function at the [fn] slot, with [argc] argument where [argv]
 * is the slot of the first argument. [ret] is the slot index of the
 * return value. if [ret] < 0 the return value will be discarded. */
PK_PUBLIC bool pkCallFunction(PKVM* vm, int fn, int argc, int argv, int ret);

/* Calls a [method] on the [instance] with [argc] argument where [argv]
 * is the slot of the first argument. [ret] is the slot index of the
 * return value. if [ret] < 0 the return value will be discarded. */
...
/* Set the attribute with [name] of the instance at the [instance] slot
 * to the value at the [value] index slot. Return true on success. */
PK_PUBLIC bool pkSetAttribute(PKVM* vm, int instance,
                              const char* name, int value);

/* Import a module with the [path] and place it at [index] slot. The
 * path sepearation should be '/'. Example: to import module "foo.bar"
 * the [path] should be "foo/bar". On failure, it'll set an error and
 * return false. */
